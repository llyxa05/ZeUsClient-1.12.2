...
public class Trajectories extends Module {
	...
	public void onRender3D(Event3DRender event) {
		try {
			if (getMC().player.inventory.getCurrentItem() != null) {
				EntityPlayerSP player = getMC().player;
				ItemStack stack = player.inventory.getCurrentItem();
				int itemMain = Item.getIdFromItem(getMC().player.getHeldItem(EnumHand.MAIN_HAND).getItem());
				int itemOff = Item.getIdFromItem(getMC().player.getHeldItem(EnumHand.OFF_HAND).getItem());
				...
				if (((itemMain == 261 || itemOff == 261) || (itemMain == 368 || itemOff == 368)
					...
					double posX = player.lastTickPosX
							+ (player.posX - player.lastTickPosX) * getMC().timer.field_194147_b
							- Math.cos(Math.toRadians(player.rotationYaw)) * 0.16F;
					double posY = player.lastTickPosY
							+ (player.posY - player.lastTickPosY) * getMC().timer.field_194147_b + player.getEyeHeight()
							- 0.1D;
					double posZ = player.lastTickPosZ
							+ (player.posZ - player.lastTickPosZ) * getMC().timer.field_194147_b
					...
					for (int i = 0; i < 2000; i++) {
						GL11.glVertex3d(posX - getMC().getRenderManager().renderPosX,
								posY - getMC().getRenderManager().renderPosY, posZ - getMC().getRenderManager().renderPosZ);
						...
						blockCollision = getMC().world.rayTraceBlocks(vec, new Vec3d(posX, posY, posZ));

						for (Entity o : getMC().world.getLoadedEntityList()) {
					...
					GL11.glEnd();
					double renderX = posX - getMC().getRenderManager().renderPosX;
					double renderY = posY - getMC().getRenderManager().renderPosY;
					double renderZ = posZ - getMC().getRenderManager().renderPosZ;
					GL11.glPushMatrix();
