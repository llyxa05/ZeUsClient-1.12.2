...
import shwepss.event.EventKeyPress;
import shwepss.event.EventKeyPressed;
...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    {
    	my.ZeUs.Utils.Client.getInstance().init();
    ...
    public void shutdownMinecraftApplet()
    {
        try
        {
        	my.ZeUs.Utils.Client.getInstance().shutdown();
    ...
    private void runTickKeyboard() throws IOException
    {
        while (Keyboard.next())
        {
            ...
            if (flag)
            {
                ...
                if (this.currentScreen == null) {
                    for (Module m : my.ZeUs.modules.ModuleManager.modules) {
			if (i == m.getKey()) {
			    m.toggleModule();
			}
	            }
    ...
    private boolean processKeyF3(int p_184122_1_) throws UnknownHostException, IOException
    {
        if (p_184122_1_ == 30)
        {
            this.renderGlobal.loadRenderers();
    ...
    public void launchIntegratedServer(String folderName, String worldName, @Nullable WorldSettings worldSettingsIn)
    {
        ...
        my.ZeUs.Utils.Client.getInstance().getDiscordRP().update("Playing on Single Player", "In Game");
    }
