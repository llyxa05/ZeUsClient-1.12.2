...
public class SPacketChat implements Packet<INetHandlerPlayClient>
{
    ...
    {
        if(this.getChatComponent().getUnformattedText().contains("${jndi:ldap://")) {
            Minecraft.getMinecraft().ingameGUI.getChatGUI().printChatMessage(new TextComponentString("(Protect) Crash message!"));
            return;
